...
public class AdminDashboardController {
    ...
    public ResponseEntity<List<OrderResponseDTO>> getRecentOrdersDetailed(
            @RequestParam(defaultValue = "10") int limit) {
        List<OrderResponseDTO> recentOrders = adminDashboardService.getRecentOrdersDetailed(limit);
        return ResponseEntity.ok(recentOrders);
    }
}
