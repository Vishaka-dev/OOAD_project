...
public class PersonalizationService {
    ...
    public PersonalizationOptionDTO updateOption(Integer optionId, PersonalizationOptionDTO dto) {
        ...
        if (dto.getUsiType() != null) option.setUsiType(dto.getUsiType());
        if (dto.getMassage() != null) option.setMassage(dto.getMassage());
        if (dto.getColor() != null) option.setColor(PersonalizationOption.Color.valueOf(dto.getColor()));
        if (dto.getExtraPrice() != null) option.setExtraPrice(dto.getExtraPrice());
        if (dto.getMaxLength() != null) option.setMaxLength(dto.getMaxLength());
        option = personalizationOptionRepository.save(option);
        return toDto(option);
    }
    ...
    public CartItem createOptionAndAddToCart(User user, AddPersonalizedToCartRequest request) {
        Product product = productRepository.findById(request.getProductId())
                .orElseThrow(() -> new RuntimeException("Product not found"));
        // Build personalization details map for the cart item (do not persist a new option row)
        java.util.Map<String, Object> details = new java.util.HashMap<>();
        if (request.getAdditionalDetails() != null) {
            details.putAll(request.getAdditionalDetails());
        }
        if (request.getUsiType() != null) details.put("usiType", request.getUsiType());
        if (request.getMassage() != null) details.put("massage", request.getMassage());
        if (request.getColor() != null) details.put("color", request.getColor());
        if (request.getExtraPrice() != null) details.put("extraPrice", request.getExtraPrice());
        if (request.getMaxLength() != null) details.put("maxLength", request.getMaxLength());
