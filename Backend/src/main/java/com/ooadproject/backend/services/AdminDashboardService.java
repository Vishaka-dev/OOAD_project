...
public class AdminDashboardService {
    ...
    public AdminDashboardStatsDTO getDashboardStats() {
        ...
        // Revenue statistics - all completed orders
        List<Order> completedOrders = orderRepository.findByStatus(Order.OrderStatus.Delivered);
        stats.setTotalRevenue(completedOrders.stream()
        ...
        stats.setLowStockProducts(inventoryRepository.countLowStockItems());
        stats.setOutOfStockProducts(inventoryRepository.countOutOfStockItems());
        stats.setActiveProducts(productRepository.count() - inventoryRepository.countOutOfStockItems());
        ...
        for (Map.Entry<LocalDate, List<Order>> entry : ordersByDate.entrySet()) {
            ...
            if (dayOrders.size() > 0) {
                report.setAverageOrderValue(
                        totalSales.divide(BigDecimal.valueOf(dayOrders.size()), 2, RoundingMode.HALF_UP)
                );
            } else {
        ...
        for (Map.Entry<Integer, List<OrderItem>> entry : itemsByProduct.entrySet()) {
            ...
            salesDTO.setCategoryName(
                    items.get(0).getProduct().getCategory() != null ?
                            items.get(0).getProduct().getCategory().getName() : "Unknown"
            );
